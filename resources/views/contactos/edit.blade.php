<x-layouts.app title="Editar Contacto" metaDescription="Editar información del contacto">
    <div style="background: white; padding: 2rem; border-radius: 8px; box-shadow: 0 2px 8px rgba(0,0,0,0.1); max-width: 600px; margin: 0 auto;">
        <h1 style="color: #2c3e50; margin-bottom: 2rem; font-size: 2rem;">Editar Contacto</h1>
        ...
        <form action="{{ route('contactos.update', $contacto) }}" method="POST">
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <textarea name="direccion" 
                          ...
                          rows="3"
                          required
                          style="width: 100%; padding: 0.75rem; border: 1px solid {{ $errors->has('direccion') ? '#e74c3c' : '#ddd' }}; border-radius: 4px; font-size: 1rem; resize: vertical;">{{ old('direccion', $contacto->direccion) }}</textarea>
                @error('direccion')
                    <span style="color: #e74c3c; font-size: 0.875rem; margin-top: 0.25rem; display: block;">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div style="display: flex; gap: 1rem; justify-content: flex-end;">
                ...
                <button type="submit" 
                        style="background-color: #f39c12; color: white; padding: 0.75rem 1.5rem; border: none; border-radius: 4px; cursor: pointer; font-size: 1rem; transition: background-color 0.3s;"
